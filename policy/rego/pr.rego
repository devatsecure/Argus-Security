...
suppressed_findings := [f |
    ...
    not f.severity in ["critical", "high"]  # Never suppress critical/high
]

# Suppress findings with low historical fix rate (never suppress critical/high)
...
# Don't block auto-fixable findings (only medium/low — critical/high always require review)
auto_fixable_findings := [f |
    f := input.findings[_]
    f.auto_fixable == true
    not f.severity in ["critical", "high"]  # Critical/high always require manual review
]
